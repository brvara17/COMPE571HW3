...
{
    class RMEEScheduler:RMScheduler
    {
        ...
        static int numberOfTasksGlobal = new int();


        public void RMEEAnalysis(List<List<string>> data)
        {
            //GetCombinationSample();
            timeToExecute = Convert.ToInt32(data[0][1]);
            numberOfTasksGlobal = Convert.ToInt32(data[0][0]);
            TaskScheduler.GetData(data, timeToExecute);
            //FindCombinations(data);
            Console.WriteLine("Running RM EE Analysis:");
            ...
            int counter = 0;
            foreach(List<int> li in taskList)
            {
                li.Insert(0, listDeadlines[counter]);
                counter++;
            }


            //Schedules all tasks in the system
            int[] arrayEDFEESchedule = this.scheduleRM(taskList, timeToExecute);
        ...
        private List<List<int>> CreateLowestEDF(List<List<double>> taskPosibilities, List<List<string>> data)
        {
            ...
            do
            {
                ...
                systemFailed = CheckRM(tasksInSystem);
            }
            while (!systemFailed);
        ...
        public static bool CheckRM(List<List<int>> data)
        {
            double sum = 0;
            double numOfTasks = numberOfTasksGlobal;
